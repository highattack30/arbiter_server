...
class connection; class player; struct w_server_node;
...
struct j_recv_from_node {
	j_recv_from_node(w_server_node*, int8);


	const int8 state;
	w_server_node* from;
};
struct j_send_to_node {
	j_send_to_node(w_server_node*);

	WSABUF		wsa_buff;
	Stream		data;
	const w_server_node * node;
};
#endif // ifndef JOB_H
