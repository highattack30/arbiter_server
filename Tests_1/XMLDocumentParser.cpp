...
const bool XMLDocumentParser::ParseZoneXMLDocument(const char* fileName, std::vector<zone_template>& out_zones)
{
	...
	{
		if (_mainNodes[i]->tagName == "Zones")
			for (size_t j = 0; j < _mainNodes[i]->childNodes.size(); j++)
			{
				BuildZone(_mainNodes[i]->childNodes[j], out_zones);
			}
...
void XMLDocumentParser::BuildZone(XMLNode* mainNode, std::vector<zone_template>& out_zones)
{
	XMLNodeArgument* arg = nullptr;
	zone_template z_t;
	memset(&z_t, 0, sizeof z_t);

	if ((arg = mainNode->ConsumeArgument("continentId")))
	{
		z_t.continentId = atoi(arg->argumentValue.c_str());
	}
	if ((arg = mainNode->ConsumeArgument("ride")))
	{
		z_t.ride = arg->argumentValue == "True" ? true : false;
	}
	if ((arg = mainNode->ConsumeArgument("pk")))
	{
		if (arg->argumentValue == "false" || arg->argumentValue == "safe")
			z_t.pk = false;
		else
			z_t.pk = true;
	}
	if ((arg = mainNode->ConsumeArgument("vender")))
	{
		z_t.vender = arg->argumentValue == "True" ? true : false;
	}
	if ((arg = mainNode->ConsumeArgument("worldMapGuardId")))
	{
		z_t.worldMapGuardId = atoi(arg->argumentValue.c_str());
	}
	if ((arg = mainNode->ConsumeArgument("worldMapWorldId")))
	{
		z_t.worldMapWorldId = atoi(arg->argumentValue.c_str());
	}
	if ((arg = mainNode->ConsumeArgument("worldMapSectionId")))
	{
		z_t.worldMapSectionId = atoi(arg->argumentValue.c_str());
	}
	if ((arg = mainNode->ConsumeArgument("addMaxZ")))
	{
		z_t.addMaxZ = (float)atof(arg->argumentValue.c_str());
	}
	if ((arg = mainNode->ConsumeArgument("subtractMinZ")))
	{
		z_t.substractMinZ = (float)atof(arg->argumentValue.c_str());
	}
	if ((arg = mainNode->ConsumeArgument("campId")))
	{
		z_t.campId = atoi(arg->argumentValue.c_str());
	}
	if ((arg = mainNode->ConsumeArgument("maxPlayerCount")))
	{
		z_t.maxPlayerCount = atoi(arg->argumentValue.c_str());
	}
	if ((arg = mainNode->ConsumeArgument("floor")))
	{
		z_t.floor = atoi(arg->argumentValue.c_str());
	}
	if ((arg = mainNode->ConsumeArgument("restBonus")))
	{
		z_t.restExpBonus = arg->argumentValue == "True" ? true : false;
	}
	if ((arg = mainNode->ConsumeArgument("maze")))
	{
		z_t.maze = arg->argumentValue == "True" ? true : false;
	}
	if ((arg = mainNode->ConsumeArgument("ignoreObstacleShortTel")))
	{
		z_t.ignoreObstacleShortTel = arg->argumentValue == "True" ? true : false;
	}
	if ((arg = mainNode->ConsumeArgument("pcMoveCylinder")))
	{
		z_t.pcMoveCylinder = arg->argumentValue == "True" ? true : false;
	}

	out_zones.push_back(std::move(z_t));
}
void XMLDocumentParser::BuildItems(XMLNode * mainnode, std::vector<const item_template *>& out_items)
{
	...
	for (size_t j = 0; j < mainnode->childNodes.size() - 1; j++)
	{
		...
		for (size_t i = 0; i < node->_arguments.size(); i++)
		{
			...
			{
				out->bind_type = itemBoundTypeDictionary[(const char*)arg->argumentValue.c_str()];
			}
...
void XMLDocumentParser::BuildNpcTemplate(XMLNode * tNode, int huntingZoneId, std::vector<const npc_template *>& out_data)
{
	...
	for (size_t i = 0; i < tNode->childNodes.size(); i++)
	{
		...
		else if (sub && sub->tagName == "Stat")
		{
			...
			{
				outTemplate->stats.base_max_hp = atoi(arg->argumentValue.c_str());
			}
		}
...
{
	itemBoundTypeDictionary.insert(std::pair<std::string, e_item_bound_type>("None", BIND_NONE));
	itemBoundTypeDictionary.insert(std::pair<std::string, e_item_bound_type>("Equip", BIND_ON_EQUIPE));
	itemBoundTypeDictionary.insert(std::pair<std::string, e_item_bound_type>("Pickup", BIND_ON_PICK_UP));
	itemBoundTypeDictionary.insert(std::pair<std::string, e_item_bound_type>("Loot", BIND_ON_LOOT));
...
XMLDocumentParser::XMLNodeArgument * XMLDocumentParser::XMLNode::GetArgument(STRING argumentName)
{
	for (size_t i = 0; i < _arguments.size(); i++)
		if (_arguments[i] && _arguments[i]->argumentName == argumentName)
			return _arguments[i];

	return nullptr;
}

void XMLDocumentParser::XMLNode::reset_args()
{
	for (size_t i = 0; i < _arguments.size(); i++)
	{
		if (_arguments[i])
			_arguments[i]->consumed = 0x00;
	}
}
